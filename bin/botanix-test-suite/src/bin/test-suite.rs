...
use botanix_test_suite::{
    config::CliArgs, context::GlobalContext, it_info_print, it_warn_print,
    server::TestServer,
};
use reth_tracing::{
    tracing_subscriber::filter::LevelFilter, LayerInfo, LogFormat, RethTracer,
    Tracer,
};
use std::sync::Arc;
...
async fn main() -> Result<()> {
    ...
    let log_filter = std::env::var("RUST_LOG")
        .unwrap_or_else(|_| "test_suite=info".to_string());
    ...
    let resources_ctx = Arc::new(
        GlobalContext::new(cli_args)
            .await
            .context("Failed to create global context")?,
    );
    ...
    let result =
        tokio::spawn(async move { suite_test_server.start(test_to_run).await });
