...
pub const MINT_CONTRACT_ADDRESS: &str =
...
pub fn spawn_child_process(
    ...
    let (child, _, _) =
        spawn_child_process_internal(scope, command, args, process_pwd, false)?;
    Ok(child)
}
        ...
        tokio::task::spawn(async move {
            ...
            while let Ok(Some(line)) = lines.next_line().await {
                ...
                if forward_messages {
                    ...
                    {
                        tracing::error!(
                            "Failed to send stdout over channel: {}",
                            e
                        );
                    }
                }
        ...
        tokio::task::spawn(async move {
            ...
            while let Ok(Some(line)) = lines.next_line().await {
                ...
                if forward_messages {
                    ...
                    {
                        tracing::error!(
                            "Failed to send stderr over channel: {}",
                            e
                        );
                    }
                }
...
pub async fn create_botanix_eth_client(
    ...
    let mint_contract_address: EtherAddress = MINT_CONTRACT_ADDRESS
        .parse()
        .context("Must be a valid ethereum address")?;
...
pub trait TemplateWriter {
        ...
        Self: askama::Template + serde::Serialize,
    {
        let rendered_template =
            self.render().context("Failed to render dynamic template")?;
