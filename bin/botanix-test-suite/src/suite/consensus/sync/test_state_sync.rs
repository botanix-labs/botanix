...
use crate::{
    ...
    suite::consensus::{
        common::{
            ...
            events::SEND_AMOUNT, poa_node::Notifications,
        },
...
pub async fn test_state_sync(
    ...
    while let Ok(notification) = rx.recv().await {
            ...
            notification
        {
            ...
            let snapshots =
                botanix_db_provider.get_snapshots().unwrap_or_default();
            if snapshots.len() >= REQUIRED_SNAPSHOTS {
                let first_snapshot_block_id =
                    snapshots.first().unwrap().height();
                ...
                let mut snapshot_chunks_data: Vec<(u64, Vec<Vec<u8>>)> =
                    Vec::new();
                for id in chunk_ids {
                    let chunk = botanix_db_provider
                        .get_chunk_by_id(id)
                        .unwrap()
                        .unwrap();
                ...
                for (ending_block, chunk_data) in snapshot_chunks_data {
                    for chunk in chunk_data {
                        let mut sealed_blocks_saved: Vec<BlockWithSenders> =
                            Vec::new();
                        let sealed_blocks = data_parser
                            .decode::<BlockWithSenders>(&chunk)
                            .await;
                        match sealed_blocks {
                            Ok(sealed_block) => {
                                it_info_print!(
                        ...
                        assert!(
                            sealed_blocks_saved.last().unwrap().header().number
                                == ending_block
                        );
                    }
                }
    ...
    let (trusted_block_height, trusted_block_hash) =
        if let Some(cometbft_rpc_clients) =
            suite.local_context.cometbft_rpc_clients.as_ref()
        {
            let cometrpc = cometbft_rpc_clients
                .get(target_member_index)
                .expect("cometbft rpc client")
                .clone();
            let cometbft_http_client =
                cometrpc.build_and_connect().expect("to be connected");
            ...
            let trusted_block_hash = cometbft_http_client
                ...
                .hash();
            it_info_print!(
                "trusted block hash at height 1",
                trusted_block_hash
            );
            let latest_block = cometbft_http_client
                .latest_block()
                .await
                .unwrap()
                .block
    ...
    let latest_botanix_block =
        botanix_eth_client.get_latest_block().await.unwrap();
    it_info_print!(
    ...
    let member_ids: Vec<u16> = member_ids[..member_ids
        .len()
        .saturating_sub(suite.global_context.syncing_instances as usize)] // remove the syncing nodes
        .to_vec();
    ...
    if let Some(cometbft_nodes_syncing) =
        suite.local_context.cometbft_nodes_syncing.as_ref()
    {
        for (_index, comet_node) in cometbft_nodes_syncing.iter() {
                ...
                suite.local_context.cometbft_processes.as_mut()
            {
                ...
                spawned_cometbft_processes
                    .push(comet_node.spawn_service().unwrap());
    ...
    let reth_db_provider_syncing_member = suite
        .local_context
        .get_reth_dbs()
        .get(member_ids.len())
        .cloned()
    ...
    loop {
        let last_block_number =
            reth_db_provider_syncing_member.last_block_number().unwrap();
