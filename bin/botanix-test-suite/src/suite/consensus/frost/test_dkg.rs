...
pub async fn dkg_flow(suite: &ConsensusIntegrationTestSuite) -> Result<(), Error> {
    ...
    for instance in 0..suite.global_context.fed_instances {
        ...
        let c = botanix_btc_server_client::BtcServerClient::connect(format!("http://localhost:{}", port))
            .await
...
pub async fn do_dkg(
    ...
    let mut queue: VecDeque<botanix_btc_server_client::DkgPayload> = VecDeque::new();

    // Kick-off the initial DKG payloads; only the coordinator will have a
    // payloads to send.
    for client in clients.iter_mut() {
        let p = client
            .get_dkg_payloads(tonic::Request::new(botanix_btc_server_client::Empty {}))
            .await
            .map_err(Error::Request)?
            .into_inner();
