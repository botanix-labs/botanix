...
pub async fn test_pegin_recovery(
    ...
    let temp_db_dir = std::env::temp_dir()
        .join(format!("pegin_recovery_test_{}", uuid::Uuid::new_v4()));
    ...
    let btc_processes =
        suite.local_context.btc_processes.as_ref().ok_or_else(|| {
            super::error::Error::TestVectorExport(
                "No BTC server processes found".to_string(),
            )
        })?;
    ...
    let pegin_recovery_port =
        suite.local_context.get_pegin_recovery_process_port();
    ...
    let client =
        botanix_pegin_recovery_client::PeginRecoveryServiceClient::connect(
            format!("http://localhost:{}", pegin_recovery_port),
        )
        .await
    ...
    for (index, db_path) in fed_key_package_paths.iter().enumerate() {
        ...
        let key_package_data = std::fs::read(db_path).map_err(|e| {
            Error::TestVectorExport(format!(
                "Failed to read key package file: {}",
                e
            ))
        })?;
        ...
        let exported_package: btcserverlib::database::ExportedKeyPackage =
            ciborium::from_reader(key_package_data.as_slice()).map_err(
                |e| {
                    Error::TestVectorExport(format!(
        ...
        client
            ...
            .import_key_share(tonic::Request::new(
                botanix_pegin_recovery_client::ImportKeyShareRequest {
                    ...
                    export: Some(
                        botanix_pegin_recovery_client::ExportedKeyPackage {
    ...
    let recover_response = client
        .clone()
        .recover_pegin(tonic::Request::new(botanix_pegin_recovery_client::RecoverPeginRequest {
    ...
    let recovery_txid = bitcoin::Txid::from_str(&recovery_result.txid)
        .expect("valid recovery txid");

    // Generate blocks to confirm the recovery transaction
    generate_blocks(&bitcoind_rpc, 2).await;
    tokio::time::sleep(Duration::from_secs(5)).await;

    let tx_info = bitcoind_rpc
        .get_raw_transaction_info(&recovery_txid, None)
        .expect("valid tx");
...
fn export_key_packages_for_all_members(
    ...
    for (index, db_path) in fed_db_paths.iter().enumerate() {
        let output_path =
            temp_db_dir.join(format!("key_package_fed_member_{}.bin", index));
