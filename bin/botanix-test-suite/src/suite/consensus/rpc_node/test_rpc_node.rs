...
pub async fn test_rpc_node(
    ...
    let rpc_tx_block_hash =
        rpc_tx_receipt.block_hash.expect("block hash to exist");
    ...
    for client in botanix_clients.iter() {
        ...
        let rpc_tx_block_number =
            rpc_tx_receipt.block_number.expect("block number to exist");
