...
use crate::{consensus::Storage, BotanixBlock};
...
use reth_provider::{
    ...
    ProviderError, TransactionVariant,
};
use std::{
    sync::{Arc, RwLock},
    time::Duration,
    ...
    BDB: SnapshotWriter + SnapshotReader + Clone + 'static,
{
    ...
    fn create_new_snapshot(
        ...
        let snapshot_id =
            self.storage.botanix_database_factory.create_new_snapshot(
                sealed_block.number,
                sealed_block.header().hash_slow(),
            )?;
    ...
    fn update_snapshot(
        ...
        self.storage.botanix_database_factory.update_snapshot(
            snapshot_id,
            block_id,
            chunk_id,
        )?;
    ...
    fn get_snapshot_size(
        ...
        let snapshot_size = self
            .storage
            .botanix_database_factory
            .get_snapshot_size(last_snapshot_id)?;
    ...
    fn append_to_chunk(
        ...
        self.storage.botanix_database_factory.append_to_chunk(
            chunk_id,
            block_number,
            data,
        )?;
    ...
    BDB: SnapshotWriter + SnapshotReader + Clone + 'static,
{
    async fn run(&mut self) -> Result<(), SnapshotManagerError> {
        ...
        let comet_rpc_client = match self
            .cometbft_rpc_factory
            .build_and_connect()
        {
            Ok(client) => client,
        ...
        let latest_block_height =
            get_latest_comet_block_height(&comet_rpc_client).await;
        ...
        let mut historical_blocks_stream = match missing_blocks {
            0 => futures::stream::empty::<
                Option<
                    RecoveredBlock<<RDB as reth_provider::BlockReader>::Block>,
                >,
            >()
            .boxed(),
            val if val > 0 && self.enable_historical_sync => {
                ...
                let historical_stream =
                    futures::stream::iter(starting_block..=latest_block_height)
                        .map(|block_number| {
                            block_client
            ...
            _ => {
                ...
                futures::stream::empty::<
                    Option<
                        RecoveredBlock<
                            <RDB as reth_provider::BlockReader>::Block,
                        >,
                    >,
                >()
                .boxed()
    ...
    BDB: SnapshotWriter + SnapshotReader + Clone + 'static,
{
    ...
    async fn process_block(
        ...
        let serialized_block =
            self.compressor.encode(block).await.map_err(|e| {
        ...
        let mut last_snapshot_id = match self.get_last_snapshot_height()? {
            Some((last_snapshot_id, last_snapshot_height)) => {
                if !state_lock.is_syncing_history()
                    && block.number < last_snapshot_height
                {
                    ...
                    return Err(
                        SnapshotManagerError::InvalidBlockHeightForSnapshot(),
                    );
                }
                last_snapshot_id
            }
    ...
    fn apply_retention_policy(&self) -> Result<(), SnapshotManagerError> {
        ...
        {
            if let Some((_, oldest_height)) = self
                .storage
                .botanix_database_factory
...
mod tests {
    ...
    use botanix_storage::{
        test_utils::create_test_provider_factory, DatabaseProviderFactoryRW,
    };
    ...
    async fn test_rw_snapshots() {
        ...
        let (snapshot_id, block_number) =
            client.get_first_snapshot_height().unwrap().unwrap();
        assert!(snapshot_id == 1);
        assert!(block_number == 1);
        let (snapshot_id, block_number) =
            client.get_last_snapshot_height().unwrap().unwrap();
        assert!(snapshot_id == 1);
        assert!(block_number == 1);
        ...
        let snapshot_by_id =
            client.get_snapshot_by_id(snapshot_id).unwrap().unwrap();
    ...
    async fn test_rw_snapshots_with_chunks() {
        ...
        for chunk_id in first_block_chunks.clone() {
            client
                .update_snapshot(snapshot_id, first_block_id, chunk_id)
                .unwrap();
        }
        ...
        for chunk_id in second_block_chunks.clone() {
            client
                .update_snapshot(snapshot_id, second_block_id, chunk_id)
                .unwrap();
        }
        ...
        let combined_blocks: Vec<_> =
            first_block_chunks.chain(second_block_chunks).collect();
    ...
    async fn test_rw_chunks() {
        ...
        for block_id in block_ids.clone() {
            chunk_id = client
                .create_new_chunk(snapshot_id, block_id, chunk_data[0].clone())
                .unwrap();
        }
        assert!(chunk_id == *block_ids.end());
        assert!(
            client.get_last_chunk_id().unwrap().unwrap() == *block_ids.end()
        );
        assert!(
            client.get_first_chunk_id().unwrap().unwrap() == *block_ids.start()
        );

        let chunk_by_id =
            client.get_chunk_by_id(*block_ids.end()).unwrap().unwrap();
        assert!(chunk_by_id.snapshot_id() == snapshot_id);
        assert!(chunk_by_id.chunk_data().to_vec() == chunk_data);

        let block_id =
            client.get_chunk_block_number(chunk_id).unwrap().unwrap();
    ...
    async fn test_rw_snapshot_syncs() {
        ...
        let last_snapshot_sync_id =
            client.get_last_snapshot_sync_id().unwrap().unwrap();
        ...
        let updated_snapshot_sync_by_id =
            client.get_snapshot_sync_by_id(id).unwrap().unwrap();
