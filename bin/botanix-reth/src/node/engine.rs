...
use crate::{
    node::{engine_api::payload::BotanixPayloadTypes, BotanixNode},
    BotanixBlock, BotanixPrimitives,
};
...
/// Built payload for Botanix. This is similar to [`EthBuiltPayload`] but without sidecars (pegins, pegouts) as those
/// included into [`BotanixBlock`].
#[derive(Debug, Clone)]
pub struct BotanixBuiltPayload {
    /// The built block
    pub(crate) block: Arc<SealedBlock<BotanixBlock>>,
...
impl BuiltPayload for BotanixBuiltPayload {
    type Primitives = BotanixPrimitives;

    fn block(&self) -> &SealedBlock<BotanixBlock> {
        self.block.as_ref()
    }
...
// impl From<EthBuiltPayload> for BotanixBuiltPayload {
...
// pub struct BotanixPayloadBuilder<Inner> {
//     inner: Inner,
// }

// impl<Inner> PayloadBuilder for BotanixPayloadBuilder<Inner>
// where
