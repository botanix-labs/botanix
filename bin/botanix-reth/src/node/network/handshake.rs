...
/// The Binance Smart Chain (Botanix) P2P handshake.
#[non_exhaustive]
pub struct BotanixHandshake;

impl BotanixHandshake {
    ...
    pub async fn upgrade_status(
        ...
        if negotiated_status.version > EthVersion::Eth66 {
            ...
            match UpgradeStatus::decode(&mut their_msg.as_ref()).map_err(|e| {
                debug!("Decode error in Botanix handshake: msg={their_msg:x}");
...
impl EthRlpxHandshake for BotanixHandshake {
    fn handshake<'a>(
        &'a self,
        ...
        Box<
            dyn Future<Output = Result<UnifiedStatus, EthStreamError>>
                + 'a
                + Send,
        >,
    > {
