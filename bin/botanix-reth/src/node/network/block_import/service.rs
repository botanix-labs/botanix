...
use crate::{
    ...
    node::{engine_api::payload::BotanixPayloadTypes, network::BotanixNewBlock},
...
pub(crate) type BlockMsg = NewBlockMessage<BotanixNewBlock>;

/// Import outcome for a block
pub(crate) type Outcome = BlockImportOutcome<BotanixNewBlock>;

/// Import event for a block
pub(crate) type ImportEvent = BlockImportEvent<BotanixNewBlock>;
    ...
    Provider: BlockNumReader + Clone,
{
    ...
    engine: BeaconConsensusEngineHandle<BotanixPayloadTypes>,
    ...
    Provider: BlockNumReader + Clone + 'static,
{
    ...
    fn new_payload(&self, block: BlockMsg, peer_id: PeerId) -> ImportFut {
        ...
        Box::pin(async move {
            ...
            let payload = BotanixPayloadTypes::block_to_payload(sealed_block);
...
mod tests {
    ...
    impl TestFixture {
        ...
        where
            F: Fn(&BlockImportEvent<BotanixNewBlock>) -> bool,
        {
            let block_msg = create_test_block();
    ...
    fn create_test_block() -> NewBlockMessage<BotanixNewBlock> {
        let block = BotanixBlock {
        ...
        let new_block = BotanixNewBlock(NewBlock { block, td: U128::from(1) });
    ...
    async fn handle_engine_msg(
        mut from_engine: mpsc::UnboundedReceiver<BeaconEngineMessage<BotanixPayloadTypes>>,
