...
use crate::node::network::BotanixNewBlock;

pub mod handle;
pub mod service;

#[derive(Debug)]
pub struct BotanixBlockImport {
    handle: ImportHandle,
}

impl BotanixBlockImport {
    pub fn new(handle: ImportHandle) -> Self {
        Self { handle }
    }
}

impl BlockImport<BotanixNewBlock> for BotanixBlockImport {
    fn on_new_block(&mut self, peer_id: PeerId, incoming_block: NewBlockEvent<BotanixNewBlock>) {
