...
pub fn create_cometbft_factory(poa_cfg: &PoaNodeArgs) -> HttpCometBFTRpcClientFactory {
    let cometbft_rpc_factory = HttpCometBFTRpcClientFactory::default()
            .with_url(&format!("http://{}:{}", poa_cfg.cometbft_rpc_host, poa_cfg.cometbft_rpc_port));
