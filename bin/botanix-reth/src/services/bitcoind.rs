...
fn create_bitcoind_client(bitcoind_config: &BitcoindConfig) -> eyre::Result<(BitcoindClient, BitcoindClientFactory)> {
...
fn get_bitcoind_config(poa_cfg: &PoaNodeArgs, bitcoind_cfg: &BitcoindArgs, is_primary: bool) -> eyre::Result<BitcoindConfig> {
...
pub async fn setup_bitcoind_client(
    ...
    let primary_bitcoind_config = get_bitcoind_config(poa_cfg, bitcoind_cfg, true)?;
    let (primary_bitcoind_client, primary_bitcoind_factory) = create_bitcoind_client(&primary_bitcoind_config)?;

    let secondary_bitcoind_config = get_bitcoind_config(poa_cfg, bitcoind_cfg, false)?;
    let (secondary_bitcoind_client, _secondary_bitcoind_factory) = create_bitcoind_client(&secondary_bitcoind_config)?;

    let fallback_client = FallbackBitcoindClient::new(vec![
