...
use alloy_primitives::{
    keccak256, BlockNumber, Bytes, ChainId, B256, B512, U256,
};
use alloy_rlp::{Decodable, RlpDecodable, RlpEncodable};
use core::cmp::Ordering;
use revm::precompile::{
    secp256k1, u64_to_address, PrecompileError, PrecompileOutput,
    PrecompileResult, PrecompileWithAddress,
};

/// Double sign evidence validation precompile for Botanix.
...
fn double_sign_evidence_validation_run(
    ...
    let Ok(evidence) = DoubleSignEvidence::decode(&mut input.iter().as_ref())
    else {
        return revert();
    };

    let Ok(header1) = Header::decode(&mut evidence.header_bytes1.as_ref())
    else {
        return revert();
    };

    let Ok(header2) = Header::decode(&mut evidence.header_bytes2.as_ref())
    else {
        return revert();
    };

    // basic check
    if header1.number.to_be_bytes().len() > 32
        || header2.number.to_be_bytes().len() > 32
    {
        return Err(BotanixPrecompileError::DoubleSignInvalidEvidence.into());
    }
    ...
    if header1.extra.len() < EXTRA_SEAL_LENGTH
        || header2.extra.len() < EXTRA_SEAL_LENGTH
    {
        return Err(BotanixPrecompileError::DoubleSignInvalidEvidence.into());
    }
    ...
    let Ok(addr1) = secp256k1::ecrecover(sig1, recid1, &msg_hash1) else {
        return revert();
    };

    let recid2 = sig2[64];
    let sig2 = <&B512>::try_from(&sig2[..64]).unwrap();
    let Ok(addr2) = secp256k1::ecrecover(sig2, recid2, &msg_hash2) else {
        return revert();
    };
    ...
    res[52 - header1.number.to_be_bytes().len()..]
        .clone_from_slice(&header1.number.to_be_bytes());
...
mod tests {
    ...
    fn test_double_sign_evidence_validation_run() {
        ...
        let res =
            double_sign_evidence_validation_run(&Bytes::from(input), 10_000)
                .unwrap();
    ...
    fn test_double_sign_evidence_validation_run_invalid_evidence() {
        ...
        assert_eq!(
            res,
            Ok(PrecompileOutput::new_reverted(10_000, Default::default()))
        );
    }
}
