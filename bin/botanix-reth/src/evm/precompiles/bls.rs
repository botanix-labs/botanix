...
use super::error::BotanixPrecompileError;
...
/// |    32    |      96     |      [{48}]      |
fn bls_signature_validation_run(
    input: &[u8],
    gas_limit: u64,
) -> PrecompileResult {
    ...
    let revert =
        || Ok(PrecompileOutput::new_reverted(cost, Default::default()));
    ...
    if (input_length <= msg_and_sig_length)
        || !((input_length - msg_and_sig_length)
            .is_multiple_of(BLS_SINGLE_PUBKEY_LENGTH))
    {
        return revert();
    }
    ...
    let signature = &input
        [BLS_MSG_HASH_LENGTH as usize..msg_and_sig_length as usize]
        .to_vec();
    ...
    for i in 0..pub_key_count {
        let pub_key = &pub_keys_data[i as usize
            * BLS_SINGLE_PUBKEY_LENGTH as usize
    ...
    if (pub_keys.len() == 1
        && !bls::verify(&pub_keys[0], msg_hash, signature, &BLS_DST.to_vec()))
...
fn calc_gas_cost(input: &[u8]) -> u64 {
    ...
    BLS_SIGNATURE_VALIDATION_BASE
        + BLS_SIGNATURE_VALIDATION_PER_KER * pub_key_number
}
...
mod tests {
    ...
    fn test_bls_signature_validation_with_single_key() {
        ...
        let result = match bls_signature_validation_run(
        ...
        // wrong msg hash
        let msg_hash = hex!(
            "1377c7e66081cb65e473c1b95db5195a27d04a7108b468890224bedbe1a8a6eb"
        );
        let signature = hex!("8325fccd4ff01e6e0e73de4955d3cb2c6678c6a6abfc465c2991e375c5cf68841ac7847ac51c32a26bd99828bc99f2f6082c41986097e0f6e6711e57c5bd5b18fa6f8f44bf416617cf192a2ff6d4edf0890315d87e3c04f04f0d1611b64bbe0a");
        let pub_key = hex!("a842801f14464ce36470737dc159cb13191e3ad8a49f4f3a38e6a94ea5594ff65753f74661fb7ec944b98fc673bb8230");
        let mut input = Vec::<u8>::new();
        ...
        match bls_signature_validation_run(
            &Bytes::from(input.clone()),
            100_000_000,
        ) {
            Ok(res) => assert_eq!(
