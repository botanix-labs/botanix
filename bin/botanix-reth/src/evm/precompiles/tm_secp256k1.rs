...
/// Tendermint SECP256K1 signature recover precompile for Botanix.
...
fn tm_secp256k1_signature_recover_run(
    input: &[u8],
    gas_limit: u64,
) -> PrecompileResult {
    ...
    if input_length
        != SECP256K1_PUBKEY_LENGTH
            ...
            + SECP256K1_SIGNATURE_MSGHASH_LENGTH
    {
        return Err(PrecompileError::other("invalid input"));
    }

    let public_key =
        match PublicKey::from_slice(&input[..SECP256K1_PUBKEY_LENGTH]) {
            Ok(pk) => pk,
    ...
    let sig = match ecdsa::Signature::from_compact(
        &input[SECP256K1_PUBKEY_LENGTH
            ..SECP256K1_PUBKEY_LENGTH + SECP256K1_SIGNATURE_LENGTH],
    ) {
        Ok(s) => s,
    ...
    let tm_pub_key = match public_key::PublicKey::from_raw_secp256k1(
        &input[..SECP256K1_PUBKEY_LENGTH],
    ) {
        Some(pk) => pk,
...
mod tests {
    ...
    fn test_tm_secp256k1_signature_recover_run_local_key() {
        ...
        let msg_hash = hex::decode(
            "b6ac827edff4bbbf23579720782dbef40b65780af292cc66849e7e5944f1230f",
        )
        .unwrap();

        let expect_address =
            hex::decode("fa3B227adFf8EA1706098928715076D76959Ae6c").unwrap();
    ...
    fn test_tm_secp256k1_signature_recover_run_ledger_key() {
        ...
        let msg_hash = hex::decode(
            "c45e8f0dc7c054c31912beeffd6f10f1c585606d61e252e97968cd66661c2571",
        )
        .unwrap();

        let expect_address =
            hex::decode("65a284146b84210a01add088954bb52d88b230af").unwrap();
