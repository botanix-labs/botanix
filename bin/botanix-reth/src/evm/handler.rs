//! EVM Handler related to Botanix chain
...
impl<DB: revm::database::Database, INSP> Default for BotanixHandler<DB, INSP> {
    fn default() -> Self {
        Self::new()
    }
}

impl<DB: Database, INSP> Handler for BotanixHandler<DB, INSP> {
    type Evm = BotanixEvm<DB, INSP>;
...
impl<DB, INSP> InspectorHandler for BotanixHandler<DB, INSP>
where
    DB: Database,
