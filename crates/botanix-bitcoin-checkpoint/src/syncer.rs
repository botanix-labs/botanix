...
use super::{
    chain::BitcoinCheckpointsChain, checkpoint::BitcoinCheckpoint,
    error::BitcoinCheckpointError,
};
...
pub type BitcoinHashBlockStream = Box<
    dyn Stream<Item = Result<SocketMessage, bitcoincore_zmq::Error>>
        + Send
        + Unpin
        + 'static,
>;
...
impl BitcoinCheckpointsChainSynchronizer {
    ...
    /// * `rpc` - The FallbackBitcoindClient used to fetch blockchain data
    ///
    /// # Returns
    ///
    ...
    pub fn new(
        ...
        let last_synced_height =
            checkpoints_chain.recent_height().map(|height| {
                height as u64
    ...
    async fn sync_new_blocks(
        &mut self,
    ) -> Result<Vec<SyncedCheckpointInfo>, BitcoinCheckpointError> {
        ...
        let lowest_confirmation_depth =
            self.checkpoints_chain.lowest_confirmation_depth() as u64;
        ...
        let confirmed_already = last_synced_height
            .saturating_add(1)
            .saturating_sub(lowest_confirmation_depth);
        let need_to_sync =
            confirmed_available.saturating_sub(confirmed_already);
        ...
        let top_confirmed_height =
            tip_height.saturating_sub(lowest_confirmation_depth - 1);
        ...
        let from_height =
            top_confirmed_height.saturating_sub(blocks_to_sync - 1);
        let to_height = top_confirmed_height;

        let mut synced_checkpoints = Vec::new();
        for height in from_height..=to_height {
            let confirmed_hash =
                map_rpc_error!(self.rpc, get_block_hash_rpc(height))?;
            let header = map_rpc_error!(
                self.rpc,
    ...
    E: std::fmt::Debug + std::fmt::Display,
{
    loop {
        while let Some(msg) = bitcoin_block_hash_stream.next().await {
            match msg {
                Ok(SocketMessage::Message(Message::HashBlock(
                    bitcoin_block_hash,
                    _,
                ))) => {
                    tracing::trace!(
...
async fn handle_new_blocks_sync_result(
    ...
    match result {
        ...
        Err(e) => {
            tracing::warn!(
                "Async task failed to sync bitcoin checkpoints: {e}"
            );
...
mod tests {
    ...
    mod sync_new_blocks {
        ...
        #[tokio::test]
        async fn test_no_new_blocks_does_nothing() {
            let chain = Arc::new(
            ...
            mock.expect_get_block_count_rpc().returning(|| Ok(100));
            ...
            syncer.sync_new_blocks().await.expect("sync new blocks");

            assert_eq!(chain.len(), 0);
        }
        #[tokio::test]
        async fn test_new_blocks_fewer_than_limit_fetches_exact_delta() {
            ...
            let mut mock = MockRpc::new();
            mock.expect_get_block_count_rpc().returning(|| Ok(105));

            expect_header_chain(
                &mut mock,
                98..=102,
                BitcoinBlockHash::all_zeros(),
            );
        ...
        #[tokio::test]
        async fn test_blocks_truncated_to_limit() {
            ...
            let mut mock = MockRpc::new();
            mock.expect_get_block_count_rpc().returning(|| Ok(140));
        ...
        async fn test_rpc_error_mapping() {
            ...
            let mut mock = MockRpc::new();
            mock.expect_get_block_count_rpc().return_once(|| {
                Err(botanix_btc_wallet::error::BitcoindAdapterError::NoClientsAvailable)
            });
            ...
            assert!(
                matches!(result, Err(BitcoinCheckpointError::SyncRpcError { procedure_name, .. }) if procedure_name == "get_block_count_rpc")
            );
        }

        #[tokio::test]
        async fn test_stale_block_error() {
            let chain = Arc::new(
                BitcoinCheckpointsChain::try_new(6, 4, 2)
                    .expect("create valid chain"),
            );
            ...
            mock.expect_get_block_count_rpc().returning(|| Ok(5));
            ...
            mock.expect_get_block_hash_rpc()
                .with(eq(2u64))
                .returning(move |_| Ok(h2));
            mock.expect_get_block_header_rpc()
                .with(eq(h2))
                .returning(move |_| Ok(create_header(h1)));
    ...
    mod handle_new_blocks_sync_result {
        ...
        async fn test_stale_block_resets_chain_and_height_and_requests_sync() {
            ...
            let control =
                handle_new_blocks_sync_result(result, Arc::clone(&syncer_lock))
                    .await;
    ...
    mod handle_hash_block_stream_messages {
        ...
        async fn test_handle_hash_block_message() {
            ...
            let messages =
                vec![Ok(SocketMessage::Message(Message::HashBlock(hash, 0)))];
        ...
        async fn test_handle_unexpected_message() {
            ...
            let messages =
                vec![Ok(SocketMessage::Message(Message::HashTx(txid, 0)))];
        ...
        async fn test_handle_stream_error() {
            ...
            let messages =
                vec![Err(bitcoincore_zmq::Error::Invalid256BitHashLength(0))];
    ...
    mock! {
        pub Rpc {
            fn get_block_count_rpc(&self)
                -> Result<u64, botanix_btc_wallet::error::BitcoindAdapterError>;

            fn get_block_hash_rpc(&self, height: u64)
                -> Result<BitcoinBlockHash, botanix_btc_wallet::error::BitcoindAdapterError>;

            fn get_block_header_rpc(&self, hash: &BitcoinBlockHash)
            ...
            fn get_best_block_hash_rpc(
                &self,
            ) -> Result<bitcoin::BlockHash, botanix_btc_wallet::error::BitcoindAdapterError>;
            fn get_txids_rpc(
                &self,
                h: &bitcoin::BlockHash,
            ) -> Result<Vec<bitcoin::Txid>, botanix_btc_wallet::error::BitcoindAdapterError>;
            fn get_estimate_smart_fee_rpc(
                &self,
            ) -> Result<EstimateSmartFeeResult, botanix_btc_wallet::error::BitcoindAdapterError>;
            fn get_block_info_rpc(
                &self,
                block_hash: &bitcoin::BlockHash,
            ) -> Result<GetBlockResult, botanix_btc_wallet::error::BitcoindAdapterError>;
        }
    }

    // Implement the trait needed by FallbackBitcoindClient
    ...
    fn expect_header_chain(
        ...
        for height in heights {
            ...
            mock.expect_get_block_hash_rpc()
                .with(eq(height))
                .returning(move |_| Ok(new_hash));

            mock.expect_get_block_header_rpc()
                .with(eq(new_hash))
                .returning(move |_| Ok(header));
