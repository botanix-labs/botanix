    ...
    Auth: Clone,
{
    ...
    db.update_upgrading_vote(alice, Vote::Aye, false, 1)
        .unwrap();
    let res = db
        .get_upgrading_approval_rate_ayes(min_validator_count)
        .unwrap();
    assert_eq!(res, (34, 3));
    let res = db
        .get_upgrading_approval_rate_compliance(min_validator_count)
        .unwrap();
    assert_eq!(res, (0, 3));
    ...
    db.update_upgrading_vote(eve.clone(), Vote::Abstain, false, 1)
        .unwrap();
    let res = db
        .get_upgrading_approval_rate_ayes(min_validator_count)
        .unwrap();
    ...
    db.update_upgrading_vote(eve.clone(), Vote::Aye, true, 1)
        .unwrap();
    let res = db
        .get_upgrading_approval_rate_ayes(min_validator_count)
        .unwrap();
