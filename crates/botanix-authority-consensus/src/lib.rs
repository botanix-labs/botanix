...
use reth_primitives::{BlockWithSenders, RecoveredBlock, Header, SealedBlock, SealedHeader};
...
impl Consensus<SealedBlock> for AuthorityConsensus {
    ...
    fn validate_body_against_header(&self, body: &SealedBlock::Body, header: &SealedHeader<Header>) -> Result<(),Self::Error>  {
        todo!()
    }
}

impl FullConsensus<SealedBlock> for AuthorityConsensus {
    fn validate_block_post_execution(
        &self,
        block: &BlockWithSenders,
